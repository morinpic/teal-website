...
|---|---|---|---|
| Teal Primary | `#008080` | `text-teal-primary`, `bg-teal-primary`, `border-teal-primary` | ブランドカラー、ボタン背景、ホバー状態、リンク色、フォーカスリング |
...
| `#008080` (teal-primary) | `#ffffff` (white) | 約 4.6:1 | 合格（通常テキスト） |
| `#ffffff` (white) | `#008080` (teal-primary) | 約 4.6:1 | 合格（ボタン文字） |
...
- ScrollToTop: ボタン背景
- フォーカスリング: `outline: 2px solid #008080`
- スキップリンク: 背景色
...
| カラー名 | HEX | 用途案 |
|---|---|---|
| Gold | `#d4af37` | プレミアムメニューのアクセント、特別キャンペーンバッジ |

現時点ではコードベースに `#d4af37` は使用されていない。導入時は `@theme` に `--color-gold: #d4af37` を追加し、白背景に対するコントラスト比（約 2.4:1）に注意すること。テキストとしては使用せず、装飾・アイコン・バッジ背景に限定する方針。
