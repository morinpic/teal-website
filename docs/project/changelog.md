...
### 1. ✅ Hero セクションの背景画像設定
- **対応済み**: クライアント提供のサロン写真2枚（スタイリングエリア・レセプション）を Figma で 1920×1280 にクロップし、クロスフェードスライドショー（6秒間隔）で表示。`useReducedMotion` 対応済み
- **コンポーネント**: `components/HeroSlideshow.tsx`
...
- **Hero写真は対応済み**: クライアント提供のサロン内装写真をHeroスライドショーに設定
...
### 51. ✅ Hero スクロールインジケーター縦線の削除

**対応日:** 2026-03-10

- `components/HeroContent.tsx`: Hero 下部中央の白い縦線（`h-12 w-px animate-pulse bg-white/50`）を削除
- **理由**: 「SCROLL DOWN」テキストCTAは以前に削除済みで縦線のみ残っていたが、単独では意味が伝わりにくく視覚的にも冗長なため削除

---

### 50. ✅ iOS Safari ダイナミックビューポート対応（Hero セクション）

**対応日:** 2026-03-10

- `app/page.tsx`: Hero セクションの min-height を `calc(100dvh - ...)` → `calc(100svh - ...)` に変更。`100vh` を fallback として併記（`100svh` 非対応ブラウザ向け）
- `app/layout.tsx`: `viewport` エクスポートを追加（`width`, `initialScale`, `viewportFit: 'cover'`）
- **解決した問題**: iOS Safari でスクロール時に検索バーが収縮する際、`dvh`（動的ビューポート高さ）を使用していたため Hero の高さがガタついていた。`svh`（最小ビューポート高さ＝ツールバー表示状態で固定）に変更することでレイアウトを安定化
...
*作成日: 2026年3月4日*
*最終更新: 2026年3月19日（Hero スライドショー実装、ドキュメント整理）*
